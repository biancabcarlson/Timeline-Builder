...
activity, ACH, crypto, login/IP, and security-review evidence. Clicking it
queues the event — text, detected date/time, time-known status, and source —
directly in the shared browser-local inbox, with no manual review step.
Timeline Builder imports it automatically when opened and shows the source
on the event; corrections can be made afterward in Timeline Builder's edit
mode.
