...
Turns raw, unordered investigator notes (dates + descriptions) into a numbered, chronological timeline, and flags unusually large gaps between entries. Works entirely on data you provide — nothing is bundled or assumed.
...
- **Adding a note:** if it clearly matches a known event (e.g. "added beneficiary"), the date/time fills in automatically. If it doesn't match anything, you're prompted to set the date/time yourself. Either way, the range auto-widens to include it.
- **Select event:** click "Select event" to enter edit mode, then click any entry to edit its text, move it to a different date/time (it re-sorts), or delete it (behind a confirm step).
...
The 🔒 Privacy Mode toggle (top right, shared across the suite via `localStorage`) is here for consistency with the other tools — timeline entries don't currently carry PII fields of their own, so there's nothing for it to blur.
