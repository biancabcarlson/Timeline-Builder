...
Reconstruct a defensible sequence of events by organizing account activity chronologically and connecting related evidence from logins, transactions, documents and other investigative findings.
...
On the Simulated Account page, account activity, ACH, crypto, login/IP, and
security-review evidence sync to the shared browser-local inbox
automatically as the page renders — no button to click, no review step.
Each synced item is deduped by a hash of its content, so reloading the page
never queues the same event twice. Timeline Builder imports the queue
automatically when opened and shows the source on the event; corrections
can be made afterward in Timeline Builder's own edit mode.
...
Accepts a JSON file of structured events, or a plain `.txt` file of free-text note lines using the same flexible parsing as the web demo. Outputs a chronological timeline in Markdown. JSON events may include `timeSpecified`, `type`, and `source`; those fields are preserved in the Markdown so an investigator can distinguish account evidence from a later investigator observation.
